...
/**
 * Parse a formatted phone number back to digits only
 ...
 */
export const getPhoneType = (phone: string): 'mobile' | 'landline' | 'unknown' => {
  const cleaned = parsePhone(phone);
  ...
  // Mobile prefixes (common patterns)
  ...
  // This is a simplified check - in reality, you'd want a more comprehensive database
...
/**
 * Generate a clickable phone link
 * @param phone - The phone number
 * @returns HTML anchor tag for phone number
 */
 ...
 */
export const maskPhone = (phone: string, maskChar: string = '*'): string => {
  const cleaned = parsePhone(phone);
  if (cleaned.length < 4) return phone;
  
  const visibleDigits = 4;
  const maskedLength = cleaned.length - visibleDigits;
